...
function renderJobListPC(list) {
  const container = document.getElementById("jobBoardListPC");
  if (!container) return;

  container.innerHTML = list.map(job => `
    <div class="job-board__row">
      <span class="job-board__favorite ${job.isFav ? "is-active" : ""}" data-id="${job.id}">★</span>
      <div class="job-board__dday">
            <span class="job-board__deadline">${calcDDay(job.deadline)}</span>
      </div>
      <div class="job-board__title">${job.title}</div>
      <div class="job-board__meta">
        <span class="job-board__company">${job.complexName}</span>
        <span class="job-board__divider">|</span>
            ${job.area.replace(">", ">")}
      </div>
...
function renderJobListMobile(list) {
  const container = document.getElementById("jobBoardListMobile");
  if (!container) return;
  ...
  container.innerHTML = list.map(job => `
    <div class="job-board__card" data-link="/subPage/job-detail.html?id=${job.id}">
        ...
        <div class="job-board__card-head">
            <span class="job-board__company">${job.complexName}</span>
            <h3 class="job-board__title">${job.title}</h3>
        </div>
        <span class="job-board__area">${job.area.replace(">", ">")}</span>
        <span class="job-board__salary">월급 ${job.salary.replace("월급 ", "")}(세전)</span>
        <p class="job-board__link">상세보기 ›</p>
    </div>
  `).join("");
}
...
function bindFavoriteToggle() {
  document.querySelectorAll(".job-board__favorite").forEach(el => {
    el.onclick = (e) => {
      e.stopPropagation();
...
function bindMobileCardLink() {
  document.querySelectorAll(".job-board__card").forEach(card => {
    card.onclick = (e) => {
      if (e.target.closest(".job-board__favorit")) return;
